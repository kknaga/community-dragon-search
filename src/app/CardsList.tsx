...
export default function CardsList() {
      ...
      setAllItems(
        text.split("\n").map((path) => {
          ...
          return {
            ...
            ext,
            isImage:
              ext && ["png", "webp", "svg", "jpg", "jpeg", "bmp"].includes(ext)
                ? "yes"
                : "no",
            isVideo: ext && ["mp4", "webm"].includes(ext) ? "yes" : "no",
            isAudio: ext && ["ogg"].includes(ext) ? "yes" : "no",
    ...
    propertyFiltering: {
      ...
      filteringProperties: [
        ...
        },
        {
          key: "isVideo",
          operators: ["="],
          propertyLabel: "Is video",
          groupValuesLabel: "Options",
        },
        {
          key: "isAudio",
          operators: ["="],
          propertyLabel: "Is audio",
          groupValuesLabel: "Options",
        },
        {
          key: "isJson",
    ...
    return [
      ...new Set(
        allItems.flatMap((item) =>
          item.ext && item.ext.length < 6 ? item.ext : []
        )
      ),
    ].map((ext) => ({ propertyKey: "ext", value: ext }));
  }, [allItems]);
  ...
  const otherFilterOptions = [
    ...
    { propertyKey: "isVideo", value: "yes" },
    { propertyKey: "isVideo", value: "no" },
    { propertyKey: "isAudio", value: "yes" },
    { propertyKey: "isAudio", value: "no" },
  ];
  ...
  return (
    <I18nProvider locale="en" messages={[messages]}>
      <Cards
        {...collectionProps}
        header={
          <Header variant="h3" counter={`(${filteredItemsCount})`}>
